...
void LCD_DisplayString(unsigned int line, unsigned char *ptr) {
	unsigned char lineNum = 0x80;			//set lineNum = starting command to write display memory DDRAM
	lineNum += (line * 0x40);		//add 40 to the address if wanting 2nd line else line* 0 = 0
